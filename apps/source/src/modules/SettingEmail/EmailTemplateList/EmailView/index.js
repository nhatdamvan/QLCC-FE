...
import { useEmailContext, useEmailActionContext } from "../../context/EmailTemplateContextProvider";
...
const EmailView = () => {
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await jwtAxios.delete(`emailTemplate/${itemSelected}`);
      if (response) {
        fetchSuccess();
      ...
      <Table stickyHeader className="table">
        ...
        <TableBody>
          {loading ? (
            ...
            emailTemplates.map((data) => <TableItem data={data} key={data.id} onSelectContactsForDelete={onSelectContactsForDelete} />)
          )}
        </TableBody>
      </Table>
      <AppConfirmDialog
