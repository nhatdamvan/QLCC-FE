...
import { useUserGroupContext , useUserGroupActionContext } from "../../context/UserGroupContextProvider";
...
const UserGroupView = () => {
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await jwtAxios.delete(`userGroup/${itemSelected}`);
      if (response) {
        fetchSuccess();
      ...
      <Table stickyHeader className="table">
        ...
        <TableBody>
          {loading ? (
            ...
            usersGroup.map((data) => <TableItem data={data} key={data.id} onSelectContactsForDelete={onSelectContactsForDelete}  />)
          )}
        </TableBody>
      </Table>
      <AppConfirmDialog
