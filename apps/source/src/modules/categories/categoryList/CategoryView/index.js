...
import { useCategoryContext ,useCategoryActionContext } from "../../context/CategoriesContextProvider";
...
const CategoryView = () => {
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await jwtAxios.delete(`category/${itemSelected}`);
      if (response) {
        fetchSuccess();
      ...
      <Table stickyHeader className="table">
        ...
        <TableBody>
          {loading ? (
            ...
            category.map((data) => <TableItem data={data} key={data.id} onSelectContactsForDelete={onSelectContactsForDelete} />)
          )}
        </TableBody>
      </Table>
      <AppConfirmDialog
